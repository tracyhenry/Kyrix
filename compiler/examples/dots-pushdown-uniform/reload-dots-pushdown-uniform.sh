...
for i in {1..10}; do
    ...
    cmd="select run_command_on_shards('dots_pushdown_uniform', \$\$ insert into %1\$s (id,w,h) select (id::bigint) * $i*1000 + right('%1\$s',3)::bigint, (random()*100000)::int, (random()*100000)::int from generate_series(1,(100000*$SCALE/$SHARD_COUNT)::int) id;\$\$)"
    echo "$cmd" | tee | $PSQL $PGCONN -q -t
done
