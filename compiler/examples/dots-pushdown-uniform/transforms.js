...
var dotsTransform = new Transform({
    ...
    transformFunc: function (id, w, h) {
        if (!('dots_pushdown_uniform_xscale' in plv8)) { // memoize for performance
            d3=require('d3');
            plv8.dots_pushdown_uniform_xscale = d3.scaleLinear().domain([0, 100000]).range([0, CANVAS_WIDTH]);
            plv8.dots_pushdown_uniform_yscale = d3.scaleLinear().domain([0, 100000]).range([0, CANVAS_HEIGHT]);
        };
