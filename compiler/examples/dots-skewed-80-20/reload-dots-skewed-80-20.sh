...
for i in {1..80}; do
    ...
    $PSQL $PGCONN -q -t -c "insert into dots_skewed_80_20 (id,w,h) select id, (random()*400000)::bigint, (random()*50000)::bigint, (random()*2147483648*2.0 - 2147483648)::int from generate_series(1,10000*$SCALE) id;"
done

for i in {81..100}; do
    echo `date +%s`": loading dots_skewed_80_20 data #$i of 100 (uniform)..."
    $PSQL $PGCONN -q -t -c "insert into dots_skewed_80_20 (id,w,h) select id, (random()*1000000)::bigint, (random()*100000)::bigint, (random()*2147483648*2.0 - 2147483648)::int from generate_series(1,10000*$SCALE) id;"
done
