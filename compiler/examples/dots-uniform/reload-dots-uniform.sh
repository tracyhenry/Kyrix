...
$PSQL $PGCONN -t -c "drop table if exists dots_uniform cascade; create table dots_uniform(id int, w int, h int);"

for i in {1..100}; do
    echo `date +%s`": loading dots_uniform data #$i of 100..."
    $PSQL $PGCONN -q -t -c "insert into dots_uniform (id,w,h) select id, (random()*1000000)::bigint, (random()*100000)::bigint from generate_series(1,10000*$SCALE) id;"
done
