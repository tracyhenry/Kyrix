...
var countyMapStateBoundaryTransform = new Transform("select geomstr from state",
    ...
    function (row, width, height, param) {
        ...
        ret.push(! isFinite(centroid[0]) ? 0 : centroid[0]);
        ret.push(! isFinite(centroid[1]) ? 0 : centroid[1]);
        ret.push(! isFinite(bounds[0][0]) || ! isFinite(bounds[1][0]) ? 0 : bounds[1][0] - bounds[0][0]);
        ret.push(! isFinite(bounds[0][1]) || ! isFinite(bounds[1][1]) ? 0 : bounds[1][1] - bounds[0][1]);
        ret.push(row[0]);
...
var countyMapTransform = new Transform("select * from county",
    ...
    ["id", "bbox_x", "bbox_y", "bbox_w", "bbox_h", "state_id", "name", "crimerate", "population", "geomstr"],
    true);
