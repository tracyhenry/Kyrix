...
var c1BackgroundTransform = new Transform("select * from canvas_bg where canvas_id = 1;",
    ...
    function (row, width, height, params){
        ...
        var y = Math.floor((row[0]-1) / params.colnumber[row[1]-1]) * params.blockheight[row[1]-1] + params.blockheight[row[1]-1]/2;
        var ret = [];
        ret.push(row[0]);
        ret.push(x);
        ret.push(y);
...
var c1AnimalTransform = new Transform("select * from animal;",
    ...
    function (row, width, height, params){
        ...
        var y = (parseInt(row[9])/params.canvasheight[1])*params.canvasheight[0];
        var ret = [];
        ret.push(row[0]);
        ret.push(row[1]);
        ret.push(row[2]);
...
var c3AnimalTransform = new Transform("select * from animal;",
    ...
    function (row, width, height, params){
        ...
        var y = (parseInt(row[9])/params.canvasheight[1])*params.canvasheight[2];
        var ret = [];
        ret.push(row[0]);
        ret.push(row[1]);
        ret.push(row[2]);
