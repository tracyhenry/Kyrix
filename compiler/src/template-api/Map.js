...
function Map(args) {
    ...
    if (!("rendering" in args) || !("mode" in args.rendering))
        throw new Error(
            "Constructing Map: rendering mode (rendering.mode) missing."
        );
    ...
    if (!allRenderingModes.has(args.rendering.mode))
        throw new Error("Constructing Map: unsupported rendering mode.");
    ...
    if (
        ...
        throw new Error(
            "Constructing Map: object renderer (rendering.obj.renderer) missing."
        );
    if (
    ...
    if (args.rendering.obj.bboxW <= 0 || args.rendering.obj.bboxH <= 0)
        throw new Error("Constructing Map: non-positive bbox size.");
    if (
        args.x.range != null &&
        ...
        throw new Error("Constructing Map: malformed x.range");
    if (
        args.y.range != null &&
        ...
        throw new Error("Constructing Map: malformed y.range");
    if (
        "axis" in args.rendering &&
        (args.x.range == null || args.y.range == null)
    )
        throw new Error(
            "Constructing Map: raw data domain needs to be specified for rendering an axis."
        );
    if (
        (args.x.range != null && args.y.range == null) ||
        (args.x.range == null && args.y.range != null)
        ...
        throw new Error(
            "Constructing Map: x range and y range must both be provided."
        );
