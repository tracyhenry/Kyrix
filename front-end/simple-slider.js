    ...
    function slider(orientation, scale) {
        ...
        if (scale) {
            ...
            if (orientation === top || orientation === bottom) {
                ...
                height =
                    d3Array.max(scale.range()) - d3Array.min(scale.range());
            }
        ...
        function slider(context) {
            ...
            if (fill) {
                context
                    ...
                    .attr(
                        x + "2",
                        value.length === 1 ? scale(value[0]) : scale(value[1])
                    );
            }
                    ...
                    if (value.length === 2) {
                        return i === handleIndex
                            ? handleIndex === 0
                                ? Math.min(
                                      adjustedValue,
                                      alignedValue(value[1])
                                  )
                                : Math.max(
                                      adjustedValue,
                                      alignedValue(value[0])
                                  )
                            : d;
                    } else {
