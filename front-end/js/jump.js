...
function postJump(viewId, jump) {
  ...
  function postOldLayerRemoval() {
    ...
    if (jumpType == param.literalZoomOut)
      setupZoom(
        viewId,
        Math.max(gvd.curCanvas.zoomInFactorX, gvd.curCanvas.zoomInFactorY) -
          param.eps
      );
    ...
    for (var i = 0; i < globalVar.project.views.length; i++) {
      ...
      for (var j = 0; j < nGvd.curCanvas.layers.length; j++) {
        ...
        if (!curLayer.isStatic && curLayer.fetchingScheme == "tiling")
          ...
          registerJumps(
            nViewId,
            d3.select(nViewClass + ".layerg.layer" + j).select("svg"),
            j
          );
      }
    }
...
function startJump(viewId, d, jump, optionalArgs) {
  ...
  if (jump.viewport.length > 0) {
    ...
    if ("constant" in viewportFuncRet) {
      ...
      for (var i = 0; i < numLayer; i++)
        if ("layer" + i in predDict)
          postData +=
            "&predicate" + i + "=" + getSqlPredicate(predDict["layer" + i]);
    ...
    } else throw new Error("Unrecognized new viewport function return value.");
  }
...
function registerJumps(viewId, svg, layerId) {
  ...
  shapes.each(function (p) {
        ...
        d3
          ...
          .on("end", function (d) {
            if (Math.abs(dx) > 50) {
              ...
              let visFields =
                gvd.curCanvas.layers[layerId].transform.columnNames;
              ...
              // });
              console.log(`data's (x,y) before are (${d.x}, ${d.y})`);
              let data = Object.assign(d, {});
              data.x = parseFloat(data.x) + dx;
              data.cx = parseFloat(data.x) + dx;
              data.y = parseFloat(data.y) + dy;
              data.cy = parseFloat(data.y) + dy;
              console.log(`data's (x,y) after are (${data.x}, ${data.y})`);
              ...
              console.log(
                `reverse function string for attr: ${updatedField} is: ${reverseFuncString}`
              );
              const reverseFunc = Function(reverseFuncString)();
              let width = gvd.curCanvas.w;
              let height = gvd.curCanvas.h;
              console.log(
                `width of cur layer is: ${width} and height is: ${height}`
              );
    ...
    d3.select(this).on("auxclick", function (d) {
      ...
      let kyrixDivBox = d3.select(".kyrixdiv").node().getBoundingClientRect();
      d3.select(viewClass + "#updatepopover")
    ...
    d3.select(this).on("click", function (d) {
      ...
      var popoverHeight = d3
        ...
        .node()
        .getBoundingClientRect().height;
      var kyrixDivBox = d3.select(".kyrixdiv").node().getBoundingClientRect();
      d3.select(viewClass + "#jumppopover")
        .style("left", d3.event.pageX - kyrixDivBox.left + "px")
        .style(
          "top",
